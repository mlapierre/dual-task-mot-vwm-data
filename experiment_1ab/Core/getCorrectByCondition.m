function [MOT, VWM, conditions] = getCorrectByCondition(results, blocks, trials)
    for i = 1:blocks
        if find(any(results(i,1).Condition == Condition.PerformTaskA))
            conditions{i} = 'VWM-Only';
        elseif find(any(results(i,1).Condition == Condition.PerformTaskB))
            conditions{i} = 'MOT-Only';
        elseif find(any(results(i,1).Condition == Condition.PerformBoth) && ...
                    any(results(i,1).Condition == Condition.VWMResponse1st))
            conditions{i} = 'VWMResFirst';
        elseif find(any(results(i,1).Condition == Condition.PerformBoth) && ...
                    any(results(i,1).Condition == Condition.MOTResponse1st))
            conditions{i} = 'MOTResFirst';
        else
